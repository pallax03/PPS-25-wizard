...
- Permette di giocare una partita in single player contro uno o più avversari controllati da un computer (minimo giocatori da 3 a 6).
...
- Gli utenti possono interagire con il sistema attraverso l'interfaccia grafica (GUI) per:
  ...
  - scegliere il numero di giocatori (da 3 a 6)
  - effettuare le previsioni delle prese
  - giocare le carte in mano
  - scegliere la briscola del round

## Funzionali
- La configurazione della partita deve essere personalizzabile, permettendo di scegliere il numero di giocatori (da 3 a 6).
...
- Il campo da gioco deve essere aggiornato dinamicamente in base alle azioni dei giocatori.
...
- Le carte sono suddivise in:
  ...
  - carte speciali (Wizard e Jester)
- Ogni giocatore deve poter effettuare una mossa legale durante il proprio turno, rispettando le regole del gioco originale:
  - Durante la fase di previsione, ogni giocatore deve poter fare una previsione delle prese che intende fare durante il round in modo che la somma totale delle previsioni non sia uguale al numero di prese totali del round.
  - Durante la fase di gioco, ogni giocatore deve poter giocare una carta in mano, rispettando le regole:
    - Se il giocatore ha carte dello stesso colore della prima carta giocata nel round (colore di uscita), deve giocare una carta di quel colore.
...
- Il sistema deve gestire correttamente la fine della partita, mostrando i punteggi finali e dichiarando il vincitore.
