  ...
  def processAction(state: GameState, action: GameAction): Either[GameError, GameState] = (state, action) match
    case (currentState: GameState.Bidding, GameAction.PlaceBid(playerId, bid)) =>
      if playerId == currentState.currentPlayer then
        BiddingRules.processBid(bid, currentState.currentBids, playerId, currentState.core.round, currentState.core.players.toList.size) match
          ...
          case Right(bids) =>
            Right(
              GameState.Bidding(
                ...
                currentBids = bids,
                currentPlayer = currentState.core.players.nextAfter(playerId).getOrElse(currentState.currentPlayer)
              )
            )
      else
        Left(NotYourTurn)
